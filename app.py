...
# App layout
app.layout = html.Div([
    html.H1("Population Review Dashboard",id="app-title"),
...
def update_graphs(selected_state, selected_year):
    ...
    gender_fig = px.bar(df_gender, x='Category', y='Count',
                        ...
                        color='Category')
    #For the Animation Tried Chart
    # gender_fig.update_traces(
    #     hoverinfo='all',
    #     hovertemplate='<b>%{x}</b><br>Count: %{y}<extra></extra>',  # Custom hover info
    #     marker=dict(line=dict(width=2, color='black'))  # Optional: make borders visible
    # )
    ...
    # ethnicity_males_fig.update_traces(hoverinfo='all',
    #                                   hovertemplate='<b>%{label}</b><br>Count: %{value}<extra></extra>')
    #For the animatation tried
    
    ethnicity_males_fig.update_layout(
        title_x=0.5,
        title_font=dict(color='black') , # Set title color to green
    ...
    ethnicity_females_fig.update_layout(
        title_x=0.5,
        title_font=dict(color='black')
    ...
    ethnicity_other_fig.update_layout(
        title_x=0.5,
        title_font=dict(color='black')  # Set title color to purple
    )
    migration_data = {
    ...
    migration_fig = px.area(df_migration, x='Status', y='Count',
                            ...
                            labels={'Count': 'Population'},
                            color='Status')
    migration_fig.update_layout(
        title_x=0.5,
        title_font=dict(color='black')  # Set title color to orange
    )
